...
-- policy. Runtime success is still only a candidate result: invalid opening
-- negotiation actions and impossible frozen-portfolio orders converge to the
-- same deterministic defaults in both repositories.
...
BEGIN
    ...
    ELSIF v_task.task_kind = 'arena.negotiate' THEN
        ...
        ELSIF v_action_name = 'accept'
              AND (
                  v_task.input_snapshot -> 'latestCounterpartyQuote' IS NULL
                  OR v_task.input_snapshot -> 'latestCounterpartyQuote'
                      = 'null'::JSONB
              ) THEN
            v_rejection_reason := 'counterparty_proposal_required';
        ELSIF (v_task.input_snapshot ->> 'remainingTurns')::INTEGER <= 1
              AND v_action_name = 'propose' THEN
