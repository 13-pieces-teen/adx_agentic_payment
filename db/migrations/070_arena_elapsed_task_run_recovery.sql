...
-- A coordinator that resumed the migration-069 Run before the Task Factory
-- recovery fix could fail again because the already-frozen task deadline had
-- elapsed. Requeue only that proven follow-up state: the same precision repair
-- exists, all market results are projected, and at least one frozen task is
...
WITH recoverable_run AS (
    ...
    WHERE run.status = 'failed'
      AND run.safe_error_code = 'runtime_valueerror'
