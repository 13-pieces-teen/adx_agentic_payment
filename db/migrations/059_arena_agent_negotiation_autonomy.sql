...
-- The compatibility projection used by both fcfs.v1 and agent_a2a.v1 may
-- validate task shape and private hard limits, but it must not choose an
-- economic response for the Agent. In particular, an in-bound quote does not
-- force acceptance and an out-of-bound quote does not force a limit-price
-- counter.
...
CREATE OR REPLACE FUNCTION apply_arena_agent_task_result_fcfs_v1(
    p_result_id TEXT
)
RETURNS BOOLEAN
LANGUAGE plpgsql
