...
> 当前大厅默认 10 个 Ready 席位自动开赛、最多 100 人、8 回合。钱包准备必须等
> `arena402-g` 链上确认；2026-08-09 的隔离 100 钱包批次 P95/最慢为
> `159.019/162.425s`。高并发时可按约 2–3 分钟预期，以页面进入 `READY` 为准；
> 等待期间无需重复加入或重复授权。该钱包测试没有创建或开启比赛。
...
钱包白名单和测试币准备会等待链上确认，高并发时可能需要约 2–3 分钟；
