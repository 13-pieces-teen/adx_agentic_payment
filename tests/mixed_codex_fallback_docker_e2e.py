...
async def _evidence(game_id: str) -> dict[str, Any]:
    ...
    if [row["status"] for row in engagement_values] != [
        "rejected",
        "settlement_failed",
    ]:
        raise RuntimeError(
    ...
    if any(
        ...
        or row["runtime_status"] != "succeeded"
        or row["apply_status"] != "applied"
        or row["application_outcome"] != "candidate"
        for row in task_values
    ):
        raise RuntimeError(
            f"task lacks applied Runtime result: {task_values!r}"
        )
    return {
        "game": dict(game),
...
async def main() -> None:
    ...
    with tempfile.TemporaryDirectory(
        ...
        try:
            ...
            evidence = await _evidence(game_id)
            print(
                json.dumps(
                    {
                        "gameId": game_id,
